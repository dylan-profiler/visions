...
string_nan_series = pd.Series(["To travel,", "to experience and learn:", "that is to live", np.nan])
...
geometry_series = pd.Series([wkt.loads('POINT (12 42)'), wkt.loads('POINT (100 42.723)')])
...
def test_object_to_timestamp():
    standard_typeset_test(timestamp_string_series, tenzing_timestamp)
...
def test_timestamp_to_timestamp():
    standard_typeset_test(timestamp_series, tenzing_timestamp)
