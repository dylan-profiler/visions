...
def series_handle_sparse_dtype(fn: Callable):
...
def series_handle_nulls(fn: Callable) -> Callable:
...
def _contains_instance_attrs(
    series: pd.Series, is_method, class_name, attrs: list, sample_size=1
) -> bool:
...
def class_name_attrs(series: pd.Series, class_name, attrs: list, sample_size=1) -> bool:
...
def isinstance_attrs(series: pd.Series, class_name, attrs: list, sample_size=1) -> bool:
